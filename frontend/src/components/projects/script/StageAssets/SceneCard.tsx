...
import { MapPin, Check, AlertCircle, Sparkles, Loader2, Upload, Trash2 } from "lucide-react";
...
export default function SceneCard({
  projectId, scene, isGenerating, onUpload, onGenerate, onDelete, variantCount,
}: Props) {
  ...
  const handleClick = () => {
    if (scene.id) {
      router.push(`/projects/${projectId}/assets/scenes/${scene.id}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="px-2 py-2.5 text-center">
        <span className="text-xs font-bold text-text-primary block truncate">{scene.name}</span>
      </div>
    </div>
  );
}
